  ...
  const deleteUser = async (id) => {
    ...
    return (
      <div className="w-[300px] bg-gradient-to-r from-gray-900 to-gray-700 text-white p-6 rounded-2xl shadow-lg transition-transform transform hover:scale-105 hover:shadow-2xl">
        ...
        <span className="inline-block mt-3 px-4 py-1 bg-gray-600 rounded-full text-xs uppercase tracking-wide">{user.role}</span>
        <button onClick={() => deleteUser(user._id)} className='float-right cursor-pointer bg-red-500 text-white px-4 py-2 rounded mt-4'>Delete</button>
      </div>
    );
  };
